    ...
    public class Application
    {
        ...
        public static PrtgResponse ReadQueue(dynamic data)
        {
            ...
            response.result.Add(new PrtgResult
            {
                channel = "Ready",
                unit = PrtgUnit.Count,
                value = ReadInt64(() => data.messages_ready)
            });
            ...
            response.result.Add(new PrtgResult
            {
                channel = "Unacked",
                unit = PrtgUnit.Count,
                value = ReadInt64(() => data.messages_unacknowledged)
            });
            ...
            response.result.Add(new PrtgResult
            {
                channel = "Consumers",
                unit = PrtgUnit.Count,
                value = ReadInt64(() => data.consumers)
            });
        ...
        private static string ReadInt64(Func<long> func)
        {
            try
            {
                return func().ToString(CultureInfo.InvariantCulture);
